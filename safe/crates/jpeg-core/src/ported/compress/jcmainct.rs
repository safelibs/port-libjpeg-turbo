...
unsafe extern "C" fn start_pass_main(mut cinfo: j_compress_ptr, mut pass_mode: J_BUF_MODE) {
    ...
    (*main_ptr).cur_iMCU_row = 0 as JDIMENSION;
    (*main_ptr).rowgroup_ctr = 0 as JDIMENSION;
    (*main_ptr).suspended = FALSE as boolean;
    (*main_ptr).pass_mode = pass_mode;
    (*main_ptr).pub_0.process_data = Some(
        process_data_simple_main
            as unsafe extern "C" fn(j_compress_ptr, JSAMPARRAY, *mut JDIMENSION, JDIMENSION) -> (),
    )
        as Option<
            unsafe extern "C" fn(j_compress_ptr, JSAMPARRAY, *mut JDIMENSION, JDIMENSION) -> (),
        >;
}
unsafe extern "C" fn process_data_simple_main(
    ...
    while (*main_ptr).cur_iMCU_row < (*cinfo).total_iMCU_rows {
        if (*main_ptr).rowgroup_ctr < DCTSIZE as JDIMENSION {
            Some(
                (*(*cinfo).prep)
                    .pre_process_data
                    .expect("non-null function pointer"),
            ...
            .expect("non-null function pointer")(
                ...
                in_row_ctr,
                in_rows_avail,
                &raw mut (*main_ptr).buffer as JSAMPIMAGE,
                &raw mut (*main_ptr).rowgroup_ctr,
        ...
        .expect("non-null function pointer")(
            cinfo, &raw mut (*main_ptr).buffer as JSAMPIMAGE
        ) == 0
        {
            if (*main_ptr).suspended == 0 {
                *in_row_ctr = (*in_row_ctr).wrapping_sub(1);
                (*main_ptr).suspended = TRUE as boolean;
            }
            return;
        }
        if (*main_ptr).suspended != 0 {
...
pub unsafe extern "C" fn jinit_c_main_controller(
    ...
    (*cinfo).main = main_ptr as *mut jpeg_c_main_controller as *mut jpeg_c_main_controller;
    (*main_ptr).pub_0.start_pass =
        Some(start_pass_main as unsafe extern "C" fn(j_compress_ptr, J_BUF_MODE) -> ())
