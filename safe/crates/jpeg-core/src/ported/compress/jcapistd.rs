...
pub unsafe extern "C" fn jpeg_write_scanlines(
    ...
    if (*cinfo).next_scanline >= (*cinfo).image_height {
        (*(*cinfo).err).msg_code = JWRN_TOO_MUCH_DATA as ::core::ffi::c_int;
        Some(
            (*(*cinfo).err)
                .emit_message
                .expect("non-null function pointer"),
    ...
    if !(*cinfo).progress.is_null() {
        (*(*cinfo).progress).pass_counter = (*cinfo).next_scanline as ::core::ffi::c_long;
        (*(*cinfo).progress).pass_limit = (*cinfo).image_height as ::core::ffi::c_long;
        Some(
            (*(*cinfo).progress)
                .progress_monitor
                .expect("non-null function pointer"),
    ...
    row_ctr = 0 as JDIMENSION;
    Some(
        (*(*cinfo).main)
            .process_data
            .expect("non-null function pointer"),
    )
    .expect("non-null function pointer")(cinfo, scanlines, &raw mut row_ctr, num_lines);
